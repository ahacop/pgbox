...
func ExportCmdNew() *cobra.Command {
...
used independently of pgbox to run PostgreSQL with your chosen configuration.`,
  ...
  pgbox export ./my-postgres --base-image postgres:17-alpine`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return exportScaffoldNew(args[0], pgVersion, port, extList, baseImage)
		},
	}
	...
	exportCmd.Flags().StringVar(&baseImage, "base-image", "", "Base Docker image (default: postgres:<version>-bookworm)")

	return exportCmd
}

func exportScaffoldNew(targetDir, pgVersion, port, extList, baseImage string) error {
	// Validate version
	...
	if baseImage == "" {
		baseImage = fmt.Sprintf("postgres:%s-bookworm", pgVersion)
	}
